...
class GeminiAPIClient:
    ...
    async def generate_content(self, prompt: str, temperature: float = 0.3) -> Tuple[str, Optional[str]]:
        ...
        """
        if not self.api_key:
            raise ProcessingError("Gemini API key not configured")
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code != 200:
                    ...
                    raise ProcessingError(f"Gemini API request failed: {response.status_code}")
                
                result = response.json()
                
                if "candidates" not in result or not result["candidates"]:
                    raise ProcessingError("No content generated by Gemini API")
            ...
            raise ProcessingError("Gemini API request timed out")
